...
Public Class FlightData
    ...
    Function loaddropdowns()
        ...
        'req = "SELECT distinct([legtypecode])  FROM [FOSFlights]   WITH (NOLOCK)  order by legtypecode "
        ...
        If Not (IsNothing(DrpCarrier.SelectedValue)) Then
            ...
            If DrpCarrier.SelectedValue <> 0 Then
                ...
                'req = "SELECT [legtypecode]  FROM [FOSFlights]  WITH (NOLOCK)  where carrierid = " & DrpCarrier.SelectedValue & " order by legtypecode "
    ...
    Protected Sub LinkSearch_Click(sender As Object, e As EventArgs) Handles LinkSearch.Click
        ...
        If txtFrom.Text <> "" Then
            ...
            If Not IsNothing(dt) Then
                If dt.Rows.Count > 0 Then
                    ...
                    txtFrom.Text = sfrom
                Else
                    If Len(txtFrom.Text.Trim) > 3 Then da.SendEmail("CharterSales@coastalavtech.com", "pbaumgart@coastalaviationsoftware.com", "", "OptimizerStatus (dmt)" & " FlightData.aspx.vb linksearch_click Error",
        ...
        If txtTo.Text <> "" Then
            ...
            If Not IsNothing(dt) Then
                If dt.Rows.Count > 0 Then
                    ...
                    txtTo.Text = sto
                Else
                    If Len(txtTo.Text.Trim) > 3 Then da.SendEmail("CharterSales@coastalavtech.com", "pbaumgart@coastalaviationsoftware.com", "", "OptimizerStatus (dmt)" & " FlightData.aspx.vb linksearch_click Error",
        ...
        req1 = "SELECT t.RequesterName, cast(ltrim(rtrim(t.quotedtotalcost)) as money)/100 as quotedtotalcost,	f.tripnumber, f.AircraftID,  aircrafttype, "
        '20161023 - pab - use icao because iata not always populated in fosflights
        'req1 &= "f.carrierid, departureairport, arrivalairport, f.departuredategmt, departuretimegmt, flighttime, nauticalmiles, "
        req1 &= "f.carrierid, f.departureairporticao, f.arrivalairporticao, f.departuredategmt, departuretimegmt, flighttime, nauticalmiles, "
        req1 &= "duration, deadhead, paxcount, legtypecode, legratecode, legpurposecode, tripstatus, legstate, proratedrevenue  FROM [FOSFlights] f  WITH (NOLOCK)     "
        ...
        '20160329 - pab - add requester and total cost
        req1 &= "LEFT JOIN fostrips t ON f.carrierid=t.carrierid and f.tripnumber=t.tripnumber where "
        ...
        Else
            req &= "  f.carrierid = " & Session("carrierid")
        ...
        If DrpCarrier.SelectedValue <> 0 And DrpCarrier.SelectedValue <> "All" Then
            req = req & " and f.carrierid = 'abc123' "
            req = Replace(req, "abc123", DrpCarrier.SelectedValue)
        End If
        ...
        If Drpactype.SelectedValue <> "All" Then
            ...
            Select Case Drpactype.SelectedValue
                ...
                Case "B350F", "B350I"
                    req = req & " and (aircrafttype = 'B350F' or aircrafttype = 'B350I') "
        ...
        If Chkmx.Checked = True Then
            ...
            req = req & " and f.departureairporticao <> f.arrivalairporticao "
        ...
        If drpDOW.SelectedValue <> "All" Then
            req = req & " and datename(weekday, f.departuredate) =  'Monday' "
        ...
        If txtRadiusFrom.Text = 0 Then
            If txtFrom.Text <> "" Then
                ...
                req = req & " and f.departureairporticao  =  'Monday' "
        ...
        Else
            ...
            A = Replace(A, "airport", "f.departureairporticao")
            req = req & A
        End If
        ...
        If txtRadiusTo.Text = 0 Then
            If txtTo.Text <> "" Then
                ...
                req = req & " and f.arrivalairporticao   =  'Monday' "
        ...
        Else
            ...
            A = Replace(A, "airport", "f.arrivalairporticao")
            req = req & A
        End If
        ...
        If txtTripNumber.Text <> "" Then
            ...
            req = req & " and f.TripNumber = " & "'" & txtTripNumber.Text & "'"
            'End If
        End If
        ...
        SqlDataSource1.ConnectionString = ConnectionStringHelper.getglobalconnectionstring("OptimizerDataSource")
        SqlDataSource1.SelectCommand = req1 & req & "  order by CAST(f.[DepartureDateGMT] + ' ' + [DepartureTimeGMT] as datetime) "
        kkk = req1 & req & "  order by CAST(f.[DepartureDateGMT] + ' ' + [DepartureTimeGMT] as datetime) "
        ...
        Dim req2 As String
        '20160329 - pab - add requester and total cost
        req2 = "select count(*) as cnt, sum (CAST(duration as numeric(6,2) )) as duration, sum(ProRatedRevenue) as prev from [FOSFlights] f  WITH (NOLOCK)  where     " & req
        lblsql.Text = kkk
